...
class llg2_solver:
    ...
    def ex_field(self, xx0 = 1, p = 1, is_constant = False):
        self.p = p
        
        if is_constant:
            
            self.e_v = fen.Constant(("1", "0", "0"))
            
            self.dedz_v = fen.Constant(("0", "0", "0"))
            
            self.dedy_v = fen.Constant(("0", "0", "0"))
            
            self.dedx_v = fen.Constant(("0", "0", "0"))
            
        else:
        
            x, x0 = sp.symbols('x[0] x0')
            ...
            #E1 = E1.subs([(xx0, x0)])
            #dE1_dx = dE1_dx.subs([(xx0, x0)])
            
            E1_c=sp.ccode(E1)
            dE1_dx_c = sp.ccode(dE1_dx)
            
            e_v = fen.Expression((E1_c, "0", "0"), degree = 2, x0 = xx0)
            self.e_v = fen.project(e_v, self.FS)
            ...
            dedx_v = fen.Expression((dE1_dx_c, "0", "0"), degree = 2, x0 = xx0)
            self.dedx_v = fen.project(dedx_v, self.FS)
    
    def set_h_ext(self, h_x = 0, h_y = 0, h_z = 0):
        """
    ...
    def h_rest(self, m):
        ...
        vec = fen.as_vector((-self.p*(2*e1*m1.dx(0) + 2*e2*m2.dx(0) + 2*e3*m3.dx(0) + 2*m1*dedx_1 + 2*m2*dedy_1 + 2*m3*dedz_1), \
                             -self.p*(2*e1*m1.dx(1) + 2*e2*m2.dx(1) + 2*e3*m3.dx(1) + 2*m1*dedx_2 + 2*m2*dedy_2 + 2*m3*dedz_2), \
                                  -self.p*(2*m1*dedx_3 + 2*m2*dedy_3 + 2*m3*dedz_3)))
        ...
        if self.h_ext is not None:
            print_0("h_ext enabled!")
            h_total = h_total +  self.h_ext
    ...
    def solve_step(self, i):
        self.demag_pot()
        fen.solve(self.F==0, self.v, J=self.Jac, solver_parameters = { "newton_solver": { "absolute_tolerance": 1e-9, "relative_tolerance": 1e-8}})
        # self.BC
...
solver_1d.set_in_cond(ub, boundary, in_type='new', path_to_sol='/home/mnv/llg_nl/new_res/m_old/m_final.h5')


#solver_1d.e_field_from_ps(yy0=0, rr0 = 0.00001, UU0=3*2*10/3/100)

solver_1d.ex_field(xx0 = 2.5, p = -1., is_constant=False)

#solver_1d.set_h_ext("0.", "-4*3.14*2.0", "0.") #'-0.01*1000/4*tanh(x[0]+1)'

solver_1d.set_comp_params(dt = 0.001, N_f = 3000, route_0 = '/home/mnv/llg_nl/new_res/')

solver_1d.set_F()

#solver_1d.solve_step()

solver_1d.solve_2d(each_idx_write = 2)
